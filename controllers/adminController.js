...
// or just paste the function body directly into adminController.js
...
// Returns all projects that were deployed from a template
// (i.e. have a hosted_subdomain or hosted_url set)
export const getTemplateDeployments = async (req, res) => {
  try {
    ...
    // A deployment counts if it has a hosted_subdomain OR hosted_url OR is_template=1
    ...
    // fallback: show all projects that are hosted_paid
...
export const getUsers = async (req, res) => {
  try {
    ...
    // Detect optional columns dynamically
...
export const getFullUserDetail = async (req, res) => {
  try {
    ...
    const [projects, apps, payments] = await Promise.all([
      query(`
      ...
      query(`
        SELECT p.id, p.amount, p.status,
               ...
               p.created_at, COALESCE(pr.title,'') AS project_title
        FROM payments p
    ...
    console.error('getFullUserDetail:', err)
